...
function NavLink({ url, text }: NavLink) {
  ...
  return (
    <li className="flex">
      <Link
        href={url}
        className={`flex items-center mx-4 -mb-1 border-b-2 dark:border-transparent ${path === url && "dark:text-green-400 dark:border-green-400"
          }}`}
      >
        {text}
      </Link>
  ...
  return (
    <a className="flex">
      <Link
        href={url}
        onClick={handleClick}
        className={`-mx-3 block rounded-lg px-3 py-2 text-base font-semibold leading-7 text-gray-100 hover:bg-gray-900 ${path === url && "dark:text-green-400 dark:border-green-400"
          }}`}
      >
        {text}
      </Link>
